...
window.PROJECTS = [
  /* ---- 2026 ---- */
  {
    ...
    summary: '광화문에 위치한 광화문169는 한국을 깊이 경험하고자 광화문을 찾는 글로벌 방문객을 대상으로 리모델링을 진행하는 프로젝트이다.',
    body: [
      '광화문에 위치한 광화문169는 한국을 깊이 경험하고자 광화문을 찾는 글로벌 방문객을 대상으로 리모델링을 진행하는 프로젝트이다. 광화문 광장에 인접한 민간 클라이언트의 건물로서, 인근의 공공기관이나 기업 브랜드가 다루기 어려운 독창적인 콘텐츠를 담는 것을 목표로 한다. 이를 통해 K-브랜드, 문화, IP 등 한국의 자산이 세계로 확장되는 출발점이자 부싯돌 같은 역할을 지향한다.',
      '또한, 광화문 광장이 옥외광고 자유표시구역으로 지정됨에 따라 옥외광고 설치를 비롯해 온·오프라인 콘텐츠가 결합된 체험형 공간을 기획하고 있으며, 동시에 건축설계와 시공 매니지먼트까지 총괄하고 있는 프로젝트이다.'
    ],
    bodyEn: [
      'This project involves the renovation of Gwanghwa 169, located in Gwanghwamun, for global visitors seeking a deeper experience of Korea. As a privately owned building adjacent to Gwanghwamun Square, the project aims to introduce distinctive content that nearby public institutions or corporate brands are less able to offer. In doing so, it aspires to serve as a starting point and catalyst for the global expansion of Korean assets such as K-brands, culture, and intellectual property.',
      'With Gwanghwamun Square newly designated as a special district for free outdoor advertising, the project also plans an immersive space that combines outdoor media with online and offline content, while comprehensively overseeing architectural design and construction management.'
    ]
  },
  {
    id: 'tea-the-han',
  ...
  },
  {
    ...
    images: [
      ...
      'images/projects/ozx-branding-13.gif'
    ],
    name: 'OZX BRANDING',
  ...
  },
  {
    ...
    scopeCode: 'DS · SP · SD',
    scope: ['Development Strategy', 'Space BX Planning', 'Space Design'],
    summary: '묵동 G:ROUND는 중랑구 묵동 일대에 조성되는 코리빙&리테일 복합개발 프로젝트로, 청년층 주거 수요에 대응하는 동시에 지역 생활권에 활력을 더하는 새로운 주거 플랫폼을 제안한다.',
    body: [
      '묵동 G:ROUND는 중랑구 묵동 일대에 조성되는 코리빙&리테일 복합개발 프로젝트로, 청년층 주거 수요에 대응하는 동시에 지역 생활권에 활력을 더하는 새로운 주거 플랫폼을 제안한다. 기존 주거지 맥락을 반영한 공간 구성과 입주자 중심의 기획을 통해, 단순한 거주를 넘어선 통합적 주거 경험을 목표로 한다.',
  ...
  },
  {
    ...
    scopeCode: 'DS · SP · SD',
    scope: ['Development Strategy', 'Space BX Planning', 'Space Design'],
    summary: '오디세이 빌리지는 인천 중구 운북동 골든테라시티에 조성되는 리조트형 시니어 복합주거 프로젝트로, 액티브 시니어를 위한 새로운 주거 모델을 제안한다.',
    body: [
      '오디세이 빌리지는 인천 중구 운북동 골든테라시티에 조성되는 리조트형 시니어 복합주거 프로젝트로, 액티브 시니어를 위한 새로운 주거 모델을 제안한다. 나이가 들어서도 배우고, 관계 맺고, 탐험할 수 있다는 철학 아래 "Wellness Journey Town"을 공간으로 구현했다.',
      '공간 계획에서는 단지 전체를 하나의 개념으로 통일하기보다, 주변 환경과 사용자 성향에 따라 각 블록의 캐릭터를 차별화했다. 외향적인 성향을 고려한 4BL은 "Odyssey Harbor"로 계획해 외부 공원과 자연스럽게 연결되는 개방형 커뮤니티 공간을 조성했고, 내향적인 성향을 고려한 3BL은 "Odyssey Island"로 설정해 프라이빗하고 내향적인 공간 경험을 강조했다.'
    ],
    bodyEn: [
      'Odyssey Village is a resort-style senior mixed-use residential project planned for Golden Terra City in Unbuk-dong, Jung-gu, Incheon, proposing a new model for active senior living. Based on the idea that life can continue through learning, relationships, and exploration at any age, the project translates the concept of a "Wellness Journey Town" into space.',
      'Rather than applying a single concept across the entire complex, the project gives each block a distinct identity based on context and lifestyle. Odyssey Harbor is designed for more outgoing residents and encourages openness and community, while Odyssey Island is intended for more introverted residents and emphasizes privacy and individuality. The lower floors are planned with amenities that support vitality and healthy routines, while the upper floors offer calm views and premium experiences.'
    ]
  },
  {
    id: 'innocean-hq',
    ...
    scopeCode: 'DS · SP · SD',
    scope: ['Development Strategy', 'Space BX Planning', 'Space Design'],
    summary: '가장 이노션다운 워크플레이스를 만들기 위해, 임직원이 오피스에서 경험하는 EX(Employee eXperience)를 중심으로 기획한 프로젝트이다.',
    body: [
      '가장 이노션다운 워크플레이스를 만들기 위해, 임직원이 오피스에서 경험하는 EX(Employee eXperience)를 중심으로 기획한 프로젝트이다. 공간 기획의 출발점은 이노션의 정체성과 업무 특성을 깊이 이해하는 것이었다. 이를 위해 직원 FGI, 임원 인터뷰, 전사 설문조사를 진행했고, 이를 바탕으로 이노션의 밸류 키워드와 컨셉 키워드를 도출했다.',
      '"창작과 혁신이 자유자재로 일어나는 활동적이고 유기적인 워크플레이스"라는 공간 컨셉을 설정하고, 일하는 방식을 반영한 워크스페이스, 사람을 모으는 피벗스페이스, 창의적 유기성을 담은 동선, 정체성과 기능을 담은 파사드까지 기획했다. 공간경험설계 단계에서는 건축, 인테리어, 가구, 조명, 사이니지, 웨이파인딩을 아우르는 통합 공간 디자인을 통해 기획 의도를 구현하고 있다.'
    ],
    bodyEn: [
      'This project was designed around EX (Employee eXperience) to create a workplace that is uniquely Innocean. To understand Innocean\'s identity and way of working, employee FGIs, executive interviews, and company-wide surveys were conducted, leading to the development of key value and concept keywords.',
      'Based on this, the spatial concept was defined as "an active and organic workplace where creativity and innovation happen freely." The project includes workspaces that reflect Innocean\'s work style, pivot spaces that bring people together, and an integrated spatial design approach covering architecture, interiors, furniture, lighting, signage, and wayfinding. By establishing Innocean\'s own design standards, it aims to set a new benchmark for an innovative workplace that supports evolving work culture.'
    ]
  },
  {
    id: 'seoul-station-it',
    ...
    scopeCode: 'DS · SP · SD',
    scope: ['Development Strategy', 'Space BX Planning', 'Space Design'],
    summary: '"IT SEOUL 서울 — Urban Breathway 도시를 잇는 숨길이 되다"를 주제로 서울역과 주변 지역의 물리적·사회적 단절을 해소하고 도시를 연결하는 새로운 상징적 공간 프로젝트이다.',
    body: [
      '"IT SEOUL 서울 — Urban Breathway 도시를 잇는 숨길이 되다"를 주제로 서울역과 주변 지역의 물리적·사회적 단절을 해소하고 도시를 연결하며 새로운 상징적 공간으로 재탄생시키는 것을 목표로 진행되었다. 도시 연결성을 강화하고, 서울역을 대한민국 국가중앙역으로서의 정체성을 갖춘 글로벌 게이트웨이로 재구성하며, 차량 중심의 공간구조를 시민 중심의 공공공간으로 전환하는 데 중점을 두었다.',
      '"윝-서울(Upper Seoul)"과 "밑-서울(Lower Seoul)"의 수직적 조닝 컨셉을 제안했다. 통합 환승 허브, 보행 중심 서울 스트리트, 열린 광장 서울 데크, 녹지 공간 서울 힐즈, 그리고 K-브랜드 쇼룸과 문화 갤러리가 포함된다. 이 프로젝트는 서울역을 새로운 도시의 상징적 중심지로 전환하며, 물리적 공간을 넘어 도시의 가치와 정체성을 강화하는 데 기여했다.'
    ],
    bodyEn: [
